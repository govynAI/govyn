...
Plans:
- [ ] 17-01: Create `python-sdk/` package structure with `hatchling` build config, `pyproject.toml`, `govynai/__init__.py`, and `py.typed` marker
...
- [ ] 17-03: Implement `GovynAnthropic` and `GovynAsyncAnthropic` classes, `GovynBudgetExceededError`, `GovynLoopDetectedError`, and `check_proxy()` utility
